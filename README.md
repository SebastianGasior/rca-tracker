...
```bash
git clone https://github.com/SebastianGasior/rca-tracker.git
cd rca-tracker
````
...
```
# Copy to .env and update values if needed
...
1. Open your existing `README.md` in VS Code.  
...
```bash
git add README.md
git commit -m "docs: update README with project purpose and tech stack"
git push origin main
````

---

After this, your repo will be polished enough to show to recruiters **and** to deploy directly to Azure.

Would you like me to show you the exact Azure CLI steps (starting with `az login`) next, so we can deploy your container publicly?
